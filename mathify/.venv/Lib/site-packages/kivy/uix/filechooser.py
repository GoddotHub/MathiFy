...
The :class:`FileChooser` can be used as a controllers for handling multiple,
...
.. versionchanged:: 1.2.0
    ...
    In the chooser template, the `controllers` is no longer a direct reference
    but a weak-reference. If you are upgrading, you should change the notation
    `root.controllers.xxx` to `root.controllers().xxx`.
...
class FileChooserLayout(FloatLayout):
    ...
    '''
    Reference to the controllers handling this layout.
...
class FileChooserController(RelativeLayout):
    ...
    filesystem that this controllers should refer to.
